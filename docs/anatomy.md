...
│   ├── README.md
│   └── northwind-labs-interview-progress-2026-08-20.md   # demo — a progress note
│
...
| `projects/` | **Routing.** Deliverables, not context — the map tells the agent to open one project and never the folder. | Nothing in the kit writes here automatically; this is where your work lives. The freshness hook scans it for material newer than the last decision-log entry. |
...
`scripts/aios-freshness-check.sh`, wired to `SessionStart` in `.claude/settings.json`. Five
checks, read-only, silent when nothing has drifted:

1. No file in `context/` modified in the last 60 days.
2. No `reports/os-audit-*` file inside 30 days — or none at all.
3. A file in `projects/` newer than the newest entry in `decisions/log.md` — raw material that
   was never distilled. It scans `projects/` only, never `reports/`, because `reports/` is
   skill-generated by definition and scanning it would make the hook fire after every audit.
4. The `🟡 DEMO` banner still present in `context/me.md` — `/onboard` was never finished.
5. Tracked files matching `*.env`, `*token*`, or `*secret*`.
